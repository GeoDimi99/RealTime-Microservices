...
typedef struct {
    ...
    /* ZMQ output channel — created once at setup, reused across iterations */
    void *zmq_ctx;              // ZMQ context (NULL until first em_run_schedule)
    void *push_sock;            // PUSH socket bound to task_out_<id> (NULL until setup)
} activation_data_t;
